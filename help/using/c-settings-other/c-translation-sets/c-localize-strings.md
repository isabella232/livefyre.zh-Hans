...
可以自定义任何Livefyre应用程序中大多数HTML元素的文本字符串。 这样，可以灵活地将呈现的HTML元素的文本（如“发布为”按钮、“注释计数”文本或“登录”按钮）更改为任何有效的UTF-8字符串。 使用此功能为流实施添加个性化，或在应用程序中为用户群本地化语言。
...
本页列出了可为Livefyre核心应用程序自定义的所有文本字符串。
...
可用于内容流信息和显示的字符串。 列出侦听的人数、发布到应用程序的帖子数，并允许用户登录或访问其帐户信息。
...
|  | hotThreadsContentNotFoundMsg | 目前还没有足够的线索。 |
...
|---|---|---|
|  | 主持人 | 主持人 |
|  | slavercardViewProfile | 查看完整配置文件 |
| 帖子信息 | timeJustNow | 刚才 |
|  | timeMinutesAgo | 分钟前 |
...
|  | livechatOtherInstruction | 按command+Enter进行发布 |

## 响应选项 {#section_zvt_qd4_xz}

除非另行说明，否则所有登录用户均可使用。 将鼠标悬停在内容面板上即可访问。
...
| 键 | 默认文本 |  |
|---|---|---|
| Twitter @turite模态 | 提及标题 | 共享提及 |
|  | 提及子标题Twitter | 将推文共享到： |
|  | tunticeDefaultText | 我在Livefyre评论中提到过您！ |
|  | 提及确认按钮 | OK |
|  | 提及取消按钮 | 取消 |
|  | 提及错误常规 | 糟糕！ 出了什么问题！ Livefyre已收到警报。 |
|  | 提及错误无选定 | 您必须至少启用一个提及。 |
|  | 提及菜单标题 | 去看看和提及你的朋友 |
...
如果启用，则位于页面底部的所有Livefyre对话应用程序。

![](assets/strings_notifier-150x112.png)

|  | 键 | 默认文本 |
|---|---|---|
| 通知程序标签 | commentNotifier | 新建评论 |
...
| postModalPlaceholder | 你想说什么？ |
| postModalConfirationTitle | 感谢您发布！ |
| postModalConfirmationMessage | 您的帖子正在被审阅。 |
| postModalConfirmationButton | 完成 |
| title |  |
| message |  |
| editorErrorAttachmentsRequired | 需要附件 |
| editorErrorBody | 请添加一条消息 |
| editorErrorDuplicate | 尽管您很喜欢自己的便条，但您不能将其发布两次 |
| editorErrorGeneric | 出错 |
| editorErrorTitleRequired | 标题必填 |
...
| 键 | 默认文本 |
|--- |--- |
| automaticallyFollowConversions | 自动关注我加入的对话 |
| back | 返回 |
| 生物 | 个人简介 |
| create | 创建 |
...
| emailWhenSomeReplies | 当有人回复我时发送电子邮件 |
...
| forgotYourPasswordInstructions | 请在下面输入您的用户名或电子邮件地址，我们会向您发送用于更改密码的链接。 |
...
| 位置 | 位置 |
| loadingProfile | 加载配置文件 |
| newPassword | 新密码 |
| oldPassword | 旧密码 |
| on | on |
| 或 | 或 |
| passwordLinkExpired | 您单击以重置密码的链接已过期。 再次重置您的密码，我们将向您发送新链接。 |
| pleascheckEmailToComplete | 请检查您的电子邮件以完成注册。 |
| 已发布 | 已发布 |
| poweredBy | 由 |
| profileNotificationImmediate | immediate |
| profileNotificationHorywal | 每小时 |
...
| 注册 | 注册 |
| socialAccount | 社交帐户 |
| successPasswordChanged | 成功! 您的密码已更改，并且您现在已登录 |
| termsAndConditions | 条款与条件 |
| termsAndConditionsIntro | 注册后，您将接受 |
| termsOfUse | 使用条款 |
...
| passwordSymbols | 包括至少一个数字和一个符号。 |
| passwordUsername | 不包含用户名。 |
| passwordPoveporTitle | 您的密码需要： |
...
| passwordErrorMissingSymbol | 密码集中应至少包含一个符号 `!@#$%^&*()?.,<>\’;:”[]{}|` |

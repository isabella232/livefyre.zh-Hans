...
为了更好地支持来自各种来源(包括Google Maps、YouTube、Flickr、Facebook、Instagram、Spotify和Tumblr)的嵌入式媒体内容，Livefyre Apps将Embedly用作URL扩展的第三方提供商。 如果用户或审查方在帖子中包含支持的链接，则在将链接中包含的媒体发布到集合时，该媒体将会展开。

这使Livefyre应用程序能够访问Embedly支持的250多种不同的嵌入式媒体选项。

>[!NOTE]
>
>Livefyre仅扩展Embedly的完整提供程序列表的子集。 仅当提供者是Twitter、YouTube、Imgur、Vine、Wikipedia或SoundCloud时，嵌入的图像才会在HTTPS页面上扩展。 有关链接扩展或源的任何其他问题，请与您的技术客户经理联系。

本页列表了一些常用的嵌入式媒体类型及其可接受的URL模式的示例。 `Embed.ly` 不断添加新的源。有关提供程序的完整列表，请访问`https://embed.ly/embed/features/providers`。

>[!NOTE]
>
>“嵌入式”格式要求完全权限。 缩短的链接不起作用。

只有公开可查看的内容是可嵌入的。 如果您尝试嵌入非公开的内容，则指向该内容的链接将显示在博客文章中，并且该内容旁边将显示一个占位符图标。 单击该链接后，读者将从托管内容的服务转到错误消息，如仅朋友的照片的Facebook消息。 如果您注意到媒体未按预期扩展，请与您的客户经理联系。

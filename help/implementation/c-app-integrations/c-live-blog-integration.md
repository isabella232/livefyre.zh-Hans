---
description: “实时博客”允许您在报道实时事件时，从网站自己的编辑器中提供实时更新和图像。
title: 实时博客
...
“实时博客”允许您在报道实时事件时，从网站自己的编辑器中提供实时更新和图像。
...
要嵌入实时博客应用程序，请按照嵌入应用程序的过程进行操作。 请参阅[嵌入App](/help/implementation/c-livefyre-identity-comp/t-using-studio-to-connect-your-social-apps-to-your-livefyre-implementation.md)。 以下是嵌入式实时博客应用程序的外观示例。
...
| **attachmentDelegate** | *选* 项对象 | 用于指定在应用程序流中可见的媒体附件的类型。 有关详细信息，请参阅[限制媒体](../c-app-customizations/c-restrict-media.md#c_restrict_media)。 |
...
| **collectionMeta** | *必* 需字符串 | 有关集合的JWT编码的元数据。 有关详细信息，请参阅CollectionMeta对象。 |
...
| **anonymousLaggingEnabled** | 布尔值 | 定义客人用户是否具有标记内容的选项。 默认值为true。 |
...
| **editorCss** | *选* 项对象 | 用于自定义注释编辑器样式。 您可以设置编辑器字段背景颜色以及编辑器中显示的文本的字体颜色、大小和系列的样式。  例如：{background:“#ccc”，颜色：&#39;red&#39;, font:&#39;30px &quot;Helvetica Neue&quot;, Helvetica， Arial， Geneva， sans-serif&#39;} |
...
| **type** | *requiredString*  | 集合类型。 必须为livechat。 |

`CollectionMeta`对象还可能包含以下可选参数：

| 参数 | 类型 | 描述 |
|---|---|---|
| **标记** | *可选* 字符串 | 单个关键字或短语的逗号分隔列表。 在Studio中或使用搜索API按标记搜索集合。 **注意：** 虽然通过Studio添加的标记可能包含空格，但通过API输入的标记则不能。使用下划线定义将在UI中显示空格的标记。 (例如：使用Monady_Quarterback在Studio中显示Monday Quarterback。) |

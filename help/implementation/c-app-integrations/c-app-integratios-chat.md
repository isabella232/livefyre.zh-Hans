...
通过聊天，您的受众可以参与围绕实时活动的实时对话。 内容会以连续流的形式显示，鼓励快速参与展开的对话。

当前版本：聊天使用 `fyre.conv v3.0.0.`

## 集成 {#concept_0A99792A7E89403F95D082D52D600F17}

嵌入聊天应用程序遵循嵌入核心应用程序的过程，该过程在“入门”&gt;“嵌入应 [用程序”中介绍](../c-getting-started/c-implementation-process/c-using-livefyre.js-to-create-customize-and-use-apps-on-your-site.md#using-livefyre-js-create-customize-apps)。
...
如CollectionMeta Token部 [分所述](../c-getting-started/c-implementation-process/c-collectionmeta-tokent.md#collectionmeta-tokent) ,CollectionMeta是编码的JSON对象。 在上例中，JSON对象在JWT编码前采用以下格式：
...
| network | *required* string | Livefyre提供的网络名称。 例如：您 *的姓名。fyre.co.* |
| attachmentDelegate | 对象 | 用于指定在应用程序流中可见的媒体附件的类型。 有关详细信息，请参 [阅限制媒体](../c-app-customizations/c-restrict-media.md#c_restrict_media)。 |
...
| collectionMeta | *required* string | 关于集合的JWT编码的元数据。 有关 [详细信息](../c-getting-started/c-implementation-process/c-collectionmeta-tokent.md#collectionmeta-tokent) ，请参阅CollectionMeta对象。 |
...
| 校验和 | *recommended* string(recommended) | 标识CollectionMeta的当前状态。 更改此值将导致Livefyre使用CollectionMeta中的新值更新服务器上的数据。 |
| datetimeFormat | *可选字符串* Object函数 | 指定流内容的日期时间格式。 有关详细信息，请参阅自定义日期和时间戳。 |
...
| disableThirdPartyAnalytics | *可选* bBoolean | 禁用Livefyre可用于内部测量的第三方分析系统（Quantserve和Google Analytics）。 默认为 false。 |
...
## 添加事件处理函数 {#concept_E7C17EFB43F24F1A8572E60C69176C6D}

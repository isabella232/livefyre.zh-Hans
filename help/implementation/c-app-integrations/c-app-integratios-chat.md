...
title: 聊天
uuid: d6761a69-efa5-4ad3-abafa-1ba8 e17 f94
...
聊天允许观众参与实时活动的实时对话。内容以连续流的形式显示，鼓励快速参加取消折叠对话框。
...
如 [CollectionMeta令牌](../c-getting-started/c-implementation-process/c-collectionmeta-tokent.md#collectionmeta-tokent) 部分中所述，CollectionMeta是一个编码的JSON对象。在上例中，JSON对象在JWT编码之前采用以下格式：
...
| 网络 | *必需* 字符串 | Livefyre提供的网络名称。例如： *yourname. fyre. Co.* |
| AttachmentDelegate | Object | 用于指定在应用程序流中可见的媒体附件类型。有关详细信息，请参阅 [限制媒体](../c-app-customizations/c-restrict-media.md#c_restrict_media)。 |
...
| CollectionMeta | *必需* 字符串 | 有关集合的JWT编码元数据。有关更多信息，请参阅 [CollectionMeta对象](../c-getting-started/c-implementation-process/c-collectionmeta-tokent.md#collectionmeta-tokent) 。 |
...
| 校验和 | *推荐* 的字符串(建议) | 标识CollectionMeta的当前状态。更改此值将导致Livefyre使用CollectionMeta中的新值更新服务器上的数据。 |
...
| SocietthirdPartyAnalytics | *可选* BBoolean | 禁用Livefyre可用于内部测量的第三方分析系统(Quantservice和Google Analytics)。默认为false。 |
...
## 添加事件句柄 {#concept_E7C17EFB43F24F1A8572E60C69176C6D}
...
有关更多信息，以及有关可用事件的列表，请参阅 [Javascript事件](../c-app-customizations/c-javascript-events.md#c_javascript_events)。
